...
for x in range(len(public_subnets)):
  ...
  route_table_id = "${aws_route_table.littleobi-pub-rt.id}"
)
...
for x in range(len(private_subnets)):
  ...
  route_table_id = "${aws_route_table.littleobi-priv-rt.id}"
)
...
for x in range(len(secured_subnets)):
  ...
  route_table_id = "${aws_route_table.littleobi-sec-rt.id}"
)
...
print(config)
with open('../output/config.network.aws.tf.json', 'w', encoding='utf-8') as config_file_tf:
